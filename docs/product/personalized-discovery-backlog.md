...
- ~~Seat-free way to feed a listener's Spotify taste without an allowlist seat.~~ **Done (July 4, 2026, Phase 17 / PRD 45).** Listeners upload a playlist **export CSV** (Exportify, or Spotify's "Download your data"); `lib/taste-import-core.ts` parses the artists off the file and `replaceImportedProfileItems` (`lib/music.ts`) writes them as `top_artist` `music_profile_items` (`time_range: "import"`), feeding `buildProfileTerms`→`artistAffinity` exactly like the OAuth `/me/top` sync — **no Spotify API call, no allowlist seat**, works for email-only accounts. The app-token playlist-*track* read was spiked and 403s under Development Mode, which is why the import is file-based, not API-based. OAuth `/me/top` sync remains the higher-fidelity option for the ≤5 allowlisted testers.
...
## Deeper Personalization — ✅ SHIPPED (Phase 11, June 16, 2026)

> **Done — all four cycles.** The [Deeper Personalization Initiative (Epic)](deeper-personalization-prd.md) shipped end to end: **C1** PRD 18 (skips cool dimensions), **C2** PRD 19 (time-decayed per-dimension taste model), **C3** PRD 20 (cold-start + durable anonymous→account hand-off), **C4** PRD 21 (transparency, correctability & loop guardrails). See the epic's `## Implementation Status` for the per-cycle summary, and `master-roadmap.md` Phase 11 for the `**Cx shipped:**` prose. The original future-direction notes below are retained for history.
>
> **One open follow-up from this initiative** (see Remaining Follow-Up above): the **time-of-week / price / indoor-outdoor** taste dimensions were deferred in C2 and remain unbuilt.
