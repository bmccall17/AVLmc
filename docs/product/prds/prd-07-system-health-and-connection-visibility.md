...
- `lib/admin/health.ts` — `loadSystemHealth()` runs nine cheap, time-boxed, individually-degrading probes in parallel (`Promise.allSettled`), briefly cached (20s): database (`select 1` + latency), event-data freshness/empty-window, AVLgo feed reachability (status-only, body discarded), auth provider config consistency, Spotify (config conflict + 30-day profile staleness, metadata only), both cron jobs, blob storage, and Umami. Each probe returns `{ id, label, status, severity, detail, checkedAt, latencyMs? }`; one failure never breaks the page.
